...
Nx = 157; ifourierx=1; % Nodal Fourier in x (1=yes, 0=no)
Ny = 50; ifouriery=0; % Nodal Fourier in y (1=yes, 0=no)
...
tol = 1.e-9; max_iter = 400; ifnull=0;

format shorte;

Di = nu.*Da + b0; RX2=RXx; RX2(end,:)=.5*RX2(end,:); Di=RXx*Di*RXy'; Di=1./Di;
tstart=tic;
[Ub,iter,res]=cg_h(Fb,tol,max_iter,b0,nu,RXx,RXy,Bb,Grr,Grs,Gss,Dhx,Dhy,ifnull,Di);
elapsed=toc(tstart);
disp([nb iter,res,tol elapsed elapsed/iter b0 nu])

DS = 1./(nu.*XLam + b0);
tstart=tic;
[Ub,iter,res]=pcg_h(Fb,tol,max_iter,b0,nu,RXx,RXy,Bb,Grr,Grs,Gss,Dhx,Dhy,ifnull,...
                   SXx,SXy,DS);
elapsed=toc(tstart);
disp([nb iter res tol elapsed elapsed/iter b0 nu])

Zb = fdm_b(Fb,RXx,RXy,SXx,SXy,DS);
