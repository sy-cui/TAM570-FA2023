...
Rp=set_restriction(N,{'p','p','p','p','n','n'}); [Ap,Sp,Lp]=neumann_op(dim,Rp,w,D,dl); ifnull=false;
...
U{1} = 1 - Z.^2;
epsilon = 1e-5;
U{1} = U{1} + epsilon*cos(X).*sin(pi*Z);
...
for k = 1:nsteps;
    ...
    if mod(k,40) == 0 || k == 1; 
        ...
        % contourf(squeeze(Xf(:,ny,:)),squeeze(Zf(:,ny,:)),squeeze(t3w(Jf,U{kc})(:,ny,:))); axis equal
        % contourf(squeeze(Xf(:,:,2)),squeeze(Yf(:,:,2)),squeeze(t3w(Jf,V{kc})(:,:,2))); axis equal

        u_exact = 1 - Z.^2;
        ts(kk) = dt * k;
        ee(kk) = sqrt(sum(sum(sum(B.*((u_exact - U{kc}).^2 + W{kc}.^2 + V{kc}.^2)))));

        [umax vmax wmax ee(kk)]
        semilogy(ts(1:kk), ee(1:kk), '-k')
        hold on
        semilogy(ts(1:kk), epsilon.*exp(0.002234975649*ts(1:kk)), '-r')
        hold off
        xlabel("t"); ylabel("E_k");
        % colorbar;
