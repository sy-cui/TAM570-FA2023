clear all; close all; hdr;

Re=100;
nu=1./Re; alpha=nu;
N=7; Tend=40; CFL=0.15; Uchar=1;

%% System-solve parameters
ifnull=0; tol=1.e-5; max_iter=300;

Nelx_f = 3;    % Number of x elements in the front 
Nelx_b = 11;   % Number of x elements in the back
Nely = 7; 
Nelx = Nelx_f+Nelx_b+1;

save_dir = 'data_re_100';
w2f = true;
...
Nf = floor(1.5*N); [zf,wf]=zwuni(Nf); Jf=interp_mat(zf,z);
Xf = tensor3(Jf,1,Jf,X); Xa = assemble(Xf,Nf,Nelx,Nely);
Yf = tensor3(Jf,1,Jf,Y); Ya = assemble(Yf,Nf,Nelx,Nely);
...
%%   Set Dirichlet conditions onto old fields
     ...
     Ub=1-Mu; Ub(:,Esq,:)=0; Ub=qqt_op(Ub,'*',glo_num); 
     Vb= 0*X;
...
%    Diagonostics
     if mod(istep,30)==0 || istep == 1;  kk=kk+1;
          ...
          Wf = tensor3(Jf,1,Jf,Omega);

          Ua = assemble(Uf,Nf,Nelx,Nely);
          Va = assemble(Vf,Nf,Nelx,Nely);
          Ta = assemble(Tf,Nf,Nelx,Nely);
          Wa = assemble(Wf,Nf,Nelx,Nely);

          if w2f;
               fprintf("Writing to file..., t=%d, kk=%d\n", time, kk);
               dlmwrite([save_dir, sprintf("/U_%d_%d.csv", time, kk)], Ua, ",");
               dlmwrite([save_dir, sprintf("/V_%d_%d.csv", time, kk)], Va, ",");
               dlmwrite([save_dir, sprintf("/T_%d_%d.csv", time, kk)], Ta, ",");
               dlmwrite([save_dir, sprintf("/W_%d_%d.csv", time, kk)], Wa, ",");
          end;

          % disp([istep time dinf itp itu itv itt])
          % figure(10)
          % s = ['Time: ', num2str(time) ', ', num2str([umax vmax tmax])];
          % hold off; se_mesh (Xf,Yf,Uf,s); 
          % xlim([-3.5 9.5]); ylim([-3.5 3.5]); axis equal;
          % view(2); drawnow
     end;

     if tmax > 9.9; disp("Unstable!"); break; end;
...
if w2f;
     dlmwrite([save_dir, sprintf("/U_%d_%d.csv", time, kk+1)], Ua, ",");
     dlmwrite([save_dir, sprintf("/V_%d_%d.csv", time, kk+1)], Va, ",");
     dlmwrite([save_dir, sprintf("/T_%d_%d.csv", time, kk+1)], Ta, ",");
     dlmwrite([save_dir, sprintf("/W_%d_%d.csv", time, kk+1)], Wa, ",");
     dlmwrite([save_dir, "/X.csv"], Xa, ",");
     dlmwrite([save_dir, "/Y.csv"], Ya, ",");
end;
